...
// inline rename (legacy semantics, no expression rewrite until F4), default %,
// depends-on text, remove.
...
import {
  REGION_COLOR_PALETTE, addRegion, commitRegionPct, removeRegion, renameRegion,
} from './model.js';
...
function regionRow(region, ctx) {
  ...
  return h('div', { className: 'region-row' },
    ...
    h('button', {
      ...
      onclick: () => { removeRegion(ctx.model, region.name); ctx.changed({ regions: true, tasks: true }); },
    }, 'Remove'));
}
...
export function renderRegions(container, ctx) {
  ...
  container.replaceChildren(
    ...
    ...model.regions.map(r => regionRow(r, ctx)),
  );
}
